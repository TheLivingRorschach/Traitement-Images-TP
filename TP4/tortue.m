function [contour,nbpoints,energie]=tortue(img,gradient)
% 1. Initialisation
...
while (~boucle)
   ...
   nbpoints=nbpoints+1;
   contour(nbpoints,1)=p(1);
   contour(nbpoints,2)=p(2);
 
   imgcontour(contour(nbpoints,1),contour(nbpoints,2))=128;
   ...
   moyenne = 0;
   for x=1:nbpoints
       moyenne = moyenne + (gradient(contour(x,1),contour(x,2))/nbpoints);
   end
   energie = -1*moyenne;
